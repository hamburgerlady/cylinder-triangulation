function plotconic(conics,style,opts)
...
for t = 1:size(conics,2)
  ...
  if ~isnan(v(1))
    c = inv(v2m(v));
    drawconic([c(1,1),c(1,2)*2,c(2,2),c(1,3)*2,c(2,3)*2,c(3,3)],style,opts);
  end
end

function drawconic (u,style,opts)
%DRAWCONIC
%
%       drawconic (u,style)
...
%
  ...
  if (det == 0)
    ...
    b     = h/D(2,2);
    if ((a > 0) & (b > 0)),
      drawellipse (z, sqrt(a), sqrt(b), alpha, style,opts);
    else
      if (a < 0),
        tmp = a;
        a = b;
...
function drawellipse (z, a, b, alpha, style,opts)
